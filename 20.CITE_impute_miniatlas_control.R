...
cellTypes<-c("B_cells","Myeloid","STROMAL_with_lymphatics","T_cells")
ssBCs<-list()
...
#impute
cellTypes<-c("B_cells","Myeloid","STROMAL")
...
#For each sample
#eliminate that sample from the raw CITE data (part of the function?)
#impute
#save
#once done, plot imputed vs raw for each cell type
for(SSsampleName_CITE in eliminateSample){
	...
	result<-list()
		...
		for(cellType in cellTypes){
			...
			finalObject<-paste0(RobjectsDir,"RDATA_02_PROCESSED_FILTERED_",cellType,".Rdata")
			citeCells<-readRDS(finalObject)
			DefaultAssay(citeCells) <- "integrated"
			for(norm in c(FALSE)){
				cat(norm)
				cat("\n")
