...
INPUT=3921_3963_4066_Blood.b37
...
plink --bfile ${INPUT8}  --exclude Exclude-onek1k-rs_ids_correct-HRC.txt --make-bed --out ${OUTPUT8}/${OUTPUT8}


# Step 9: Make sure the all the snps are in the forward strand

INPUT9=subset_chr1to22/subset_chr1to22
...
# Get diagnostics
snpflip -b ${INPUT9}.bim -f ${REFERENCE} -o ${INPUT9}/${INPUT9}_snpflip

# Remove ambiguous SNPs
# plink --bfile ${INPUT7} --exclude ${OUTPUT7}/${OUTPUT7}_snpflip.ambiguous --make-bed --out ${OUTPUT7}/${OUTPUT7}_noambiguous --noweb


# Flip SNPs with reverse strand genotypes
plink --bfile ${INPUT7} --flip Strand-Flip-onek1k_rs_ids_correct-HRC.txt --make-bed --out ${OUTPUT7}/${OUTPUT7} --noweb
...
plink --bfile ${INPUT10} --a2-allele hrc_check5/Force-Allele1-onek1k_allele_correct-HRC.txt --make-bed --out ${OUTPUT10}/${OUTPUT10}
...
for i in {1..22}; do unzip -P QPW4jl8sDbkWWR chr_$i.zip ; done
for i in {1..22}; do unzip -P MmYLS7yPfjAcr0 chr_$i.zip ; done
gunzip *.dose.vcf.gz
grep '^#' chr1.dose.vcf > merge.vcf
grep -v '^#' chr{1..22}.dose.vcf | sed s/^.*.vcf:// >> merge.vcf 
#grep -v '^#' $projectname.b38.vcf | sed s/^.*.vcf:// >> merge.vcf 
vcf-sort merge.vcf > merge.imputed.original.sorted.vcf

grep '^#' chr1.dose.vcf > merge_overlapped.vcf
grep '^#' 4513_4530_Blood.hg38.vcf > merge_overlapped_fullheader.vcf

bedtools intersect -a $outFolder"/merge.imputed.original.sorted.vcf" -b /share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38_85/Homo_sapiens.GRCh38.85.annotated.polyA.gtf >>$outFolder"/merge_overlapped_fullheader.vcf" &
...
qsub -q short.q -b y -j y -N demuxlet -wd $logDir -pe smp 20 -V demuxlet --alpha 0.5 --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/3921_3963_4066/outs/possorted_genome_bam.bam --vcf $outFolder"/merge_overlapped_fullheader.vcf" --field GT --group-list /share/ScratchGeneral/nenbar/projects/CITE/raw_files/3921_3963_4066/outs/raw_gene_bc_matrices/GRCh38/barcodes.tsv --out $outFolder"/"$projectname"_overlapped_cleaned"
