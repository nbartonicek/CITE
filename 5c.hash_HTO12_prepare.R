...
rawDir=paste0(homeDir,"raw_files/",projectname,"/",sample,"/")
...
hto12_umi <- readRDS(paste0(rawDir,sample,"_umi_mtx.rds"))
hto12_hto_raw <- readRDS(paste0(rawDir,sample,"_hto_mtx.rds"))
...
hto12_umi_sparse <- hto12_umi[,colnames(hto12_umi) %in% colnames(hto12@data)]


#save the raw data as a data frame
raw_data=as.data.frame(as.matrix(hto12@raw.data))
#write.csv(raw_data,paste0(rawDir,"raw_counts.csv"),quote=F)

#save the data in the mtx format
#sparse.gbm <- Matrix(as.matrix(raw_data) , sparse = T )
writeMM(obj = hto12_umi_sparse, file=paste0(rawDir,"matrix.mtx"))

# save genes and cells names
write(x = rownames(raw_data), file = paste0(rawDir,"genes.tsv"))
write(x = colnames(raw_data), file = paste0(rawDir,"barcodes.tsv"))
...
hto12 <- SetAllIdent(hto12,"hto_classification_global")

class=as.character(hto12@meta.data$hto_classification_global)
class[class=="Negative"]="SNG"
class[class=="Singlet"]="SNG"
class[class=="Doublet"]="DBL"

df<-data.frame(Barcode=colnames(hto12@data),Call=class)
write.table(df,paste0(rawDir,"hash_calls.tsv"),row.names=F,quote=F,sep="\t")
