...
genes<-c("nGene","percent.mito","mm10___Rps15a",paste0("GRCh38_",c("ACTB","CD14","EPCAM","PDGFRA","FCGR3A","COL1A1","CD19","CD3E","CD4","CD8A","KRT8","KRT5", "PECAM1","PTPRC", "CD34", "ERBB2")))
...
new.cluster.ids <- c("0","B cells", "CD4 T cells", "CD4 T cells", "CD8 T cells", "epithelial cells","CD14+ Monocytes",  
    "mouse cells","mouse cells","9","10","11","12")
citeCells@ident <- plyr::mapvalues(x = citeCells@ident, from = current.cluster.ids, to = new.cluster.ids)
pdf(paste0(figDir,"vlnplot_CD44_",sampleName,".pdf"),width=12,height=6)
VlnPlot(citeCells, features = c("CITE_MHCI","CITE_CD45","mm10___Rps15a"),use.raw = TRUE, x.lab.rot=T,y.log = TRUE,ident.include=c("B cells", "CD4 T cells", "CD8 T cells", "epithelial cells","CD14+ Monocytes", "mouse cells"))
dev.off()
