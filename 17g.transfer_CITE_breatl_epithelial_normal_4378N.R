...
projectname="miniatlas_normal_epithelial"
sampleName="miniatlas_normal_epithelial"
treatment="v3"

#treatment="mt0.5_nGene7000"
figDir=paste0(homeDir,"project_results/figures/",sampleName,"_",treatment,"/")
system(paste0("mkdir -p ",figDir))
finalObject<-paste0(RobjectsDir,"RDATA_03_PROCESSED_FILTERED_RESCALEDintegrated.Rdata")
...
pdf(paste0(figDir,"CITE_original_miniatlas_normal_epithelial_feature_ADT2.pdf"),width=12,height=28)
FeaturePlot(object = citeCells1, features = row.names(citeCells1$ADT@data)[41:80], reduction="UMAPSIG30",min.cutoff = "q05", max.cutoff = "q95", ncol = 4)
dev.off()

DefaultAssay(citeCells1) <- "integrated"
...
#query@assays$integrated@data[query@assays$integrated@data<0]=0
query.anchors <- FindTransferAnchors(reference = reference, query = query, project.query=T)
save(query.anchors,file=paste0(RobjectsDir,"query.anchors_normal_epithelial_fromRNA.Rdata"))
...
save(cite,file=paste0(RobjectsDir,"imputed_cite_normal_epithelial.Rdata"))
...
saveRDS(integrated.data,file=paste0(RobjectsDir,"miniatlas_normal_epithelial_4378N-imputed-CITE.Rdata"))

pdf(paste0(figDir,"CITE_imputed_miniatlas_epithelial_feature_ADT_normal.pdf"),width=12,height=48)
FeaturePlot(object = citeCells1, features = row.names(citeCells1$ADT@data), reduction="UMAPSIG30",min.cutoff = "q05", max.cutoff = "q95", ncol = 4)
dev.off()
...
pdf(paste0(figDir,"CITE_annotated_umap_samples_normal_epithelial.pdf"),width=12,height=8)
DimPlot(object = citeCells1, reduction="UMAPSIG30",label = T,group.by="garnett_call_ext_major")
dev.off()


pdf(paste0(figDir,"CITE_annotated_umap_samples_split_normal_epithelial.pdf"),width=12,height=8)
DimPlot(object = citeCells1, reduction="UMAPSIG30",label = T,split.by="orig.ident",ncol = 4)
dev.off()



pdf(paste0(figDir,"CITE_imputed_miniatlas_normal_garnett_call_ext_major_ridgeplot.pdf"),width=8,height=32)
...
pdf(paste0(figDir,"CITE_imputed_miniatlas_normal_garnett_call_ext_major_CD3E.pdf"),width=8,height=6)
FeatureScatter(citeCells1, feature1 = "CITE-CD3", feature2 = "CD3E",group.by="garnett_call_ext_major")
dev.off()


pdf(paste0(figDir,"CITE_imputed_miniatlas_normal_garnett_call_ext_major_CD19.pdf"),width=8,height=6)
...
DoHeatmap(citeCells1, features = unique(adt.markers$gene),raster=F, assay = "ADT", angle = 90,group.by="int_cluster_major_PC_C_res.1.2") + NoLegend()
dev.off()
...
Idents(citeCells1) <- "int_cluster_major_PC_C_res.1.2"

adt.markers <- FindAllMarkers(citeCells1, assay = "ADT", only.pos = TRUE)
png(paste0(figDir,"CITE_imputed_allAtlasint_cluster_major_PC_C_res.1.2.png"),width=12,height=8,units="in",res=100)
