...
#this doesn't work with any paramater change
#query.anchors <- FindTransferAnchors(reference = reference, query = query, query.assay="integrated", project.query=T)
#save(query.anchors,file=paste0(RobjectsDir,"4497_query.anchors.Rdata"))

reference.list <- list()
reference.list[["44971"]]<-reference
reference.list[["integrated"]]<-query

integrated.anchors <- FindIntegrationAnchors(object.list = reference.list, dims = 1:30)

integrated <- IntegrateData(anchorset = integrated.anchors, dims = 1:30)
...
integrated@meta.data$orig.ident=rep(c("44971","integrated"),times=c(length(colnames(reference)),length(colnames(query))))
save(integrated,file=paste0("../project_results/Robjects/CITE_integrated_transfer_4497_miniatlas_fromObjects.Rdata"))

DefaultAssay(integrated) <- "integrated"

query <- integrated[["integrated"]]
query.anchors <- FindTransferAnchors(reference = integrated, query = query, 
    dims = 1:30)
...
predictions1 <- TransferData(anchorset = integrated.anchors, refdata = reference@assays$ADT@counts)
