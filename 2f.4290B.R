...
citeCells <- HTODemux(citeCells,assay.type = "HTO",positive_quantile =  positive_quantile, print.output = TRUE)

citeCells <- SetAllIdent(citeCells,"hto_classification_global")

pdf(paste0(figDir,"feature_plot_hash2_PQ.",positive_quantile,".",sampleName,".pdf"),width=12,height=4)
...
pdf(paste0(figDir,sampleName,"_reads_nonlog.pdf"),width=4,height=16)
...
df$marker<-factor(df$marker,levels=gsub("CITE_","",names(a)))
dfS<-df[17:24,]
otherDF<-data.frame(count=sum(df[1:17,"count"]),marker="other")
dfS<-rbind(otherDF,dfS)

pdf(paste0(figDir,sampleName,"_reads_pie.pdf"),width=4,height=4)
p<-ggplot(dfS,aes("",count,fill=marker))+geom_bar(width=1,stat="identity")
p<-p+scale_fill_brewer(palette="Blues",direction=-1)+theme_minimal()
...
#load the surface proteins

sp<-read.table("surface_proteins.txt",header=F,stringsAsFactors=F)

sp<-unique(sp$V1)

citeData<-citeCells@data
citeData<-citeData[row.names(citeData) %in% sp,]
medians<-apply(citeData,1,median)
highMeds<-sort(medians)
highMeds<-highMeds[highMeds>0]


genes<-names(highMeds)
pdf(paste0(figDir,sampleName,"_",treatment,"_surfaceProteins.pdf"),width=10,height=12)
