...
qsub -q short.q -hold_jid gzip -b y -j y -N $type$version -wd $logDir -pe smp 5 -V "CITE-seq-Count -T 5 -R1 "$inDir$version"_Thymus_"$type"_R1_001_combined.fastq.gz -R2 "$inDir$version"_Thymus_"$type"_R2_001_combined.fastq.gz -t "$inDir"tags.csv -cbf 1 -cbl 16 -umif 17 -umil 26 -o "$inDir$sampleName"_"$type".v3.tsv -cells "$cellNum" -wl "$whitelist
