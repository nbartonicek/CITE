...
rm $step1Dir/"cel_list2.txt"
(grep $step1Dir/"cel_list1.txt" -v -f $step1Dir/"failed.txt") >$step1Dir/"cel_list2.txt"
...
#process multialleles with --process-multi-alleles
...
qsub -q short.q -b y -j y -N genotypeStep2 -hold_jid genotypeStep1 -wd $logDir -pe smp 15 -P TumourProgression -V $psmetricsLine

qsub -q short.q -b y -j y -N genotypeStep3 -hold_jid genotypeStep2 -wd $logDir -pe smp 15 -P TumourProgression -V $psclassLine

qsub -q short.q -b y -j y -N genotypeStep4 -hold_jid genotypeStep3 -wd $logDir -pe smp 15 -P TumourProgression -V $otvLine
...
copyLine="cp -R /share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectname/step1/AxiomAnalysisSuiteData /share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectname/step1/OTV"
qsub -q short.q -b y -j y -N genotypeStep5 -hold_jid genotypeStep4 -wd $logDir -pe smp 1 -P TumourProgression -V $copyLine

#raw data
cleanLine="/share/ScratchGeneral/nenbar/local/lib/apt-2.10.2.2-x86_64-intel-linux/bin/apt-format-result --batch-folder $step1Dir --annotation-columns 'dbSNP_RS_ID,chromosome,position,strand' --snp-list-file $step1Dir/SNPolisher/Recommended.ps --annotation-file $annotationDir/Axiom_UKB_WCSG.na35.annot.db --export-vcf-file $outFolder/$projectname".Recommended.b37.vcf""
#OTV
otvLine="/share/ScratchGeneral/nenbar/local/lib/apt-2.10.2.2-x86_64-intel-linux/bin/apt-format-result --batch-folder $step1Dir/OTV --snp-list-file $step1Dir/OTV/OTV.keep.ps --annotation-file $annotationDir/Axiom_UKB_WCSG.na35.annot.db --export-vcf-file $outFolder/$projectname".OTV.b37.vcf""


qsub -q short.q -b y -j y -N genotypeStep6 -hold_jid genotypeStep5 -wd $logDir -pe smp 2 -P TumourProgression -V $cleanLine
qsub -q short.q -b y -j y -N genotypeStep7 -hold_jid genotypeStep5 -wd $logDir -pe smp 2 -P TumourProgression -V $otvLine


############## take the VCFs and see whether the results are the same and then overlap with transcriptome and rsID database


############## same as above but hg38
...
more $outFolder"/"$projectname".Recommended.b37.vcf" | grep -v UNKNOWNPOSITION >$outFolder"/"$projectname".Recommended.b37.clean.vcf"
more $outFolder"/"$projectname".OTV.b37.vcf" | grep -v UNKNOWNPOSITION >$outFolder"/"$projectname".OTV.b37.clean.vcf"

liftoverRecomLine="java -jar /home/nenbar/local/bin/picard.jar LiftoverVcf I=$outFolder/$projectname".Recommended.b37.clean.vcf" O=$outFolder"/"$projectname".Recommended.hg38.vcf" CHAIN=$annotationDir"/b37ToHg38.over.chain" REJECT=$annotationDir"/rejected_variants.vcf" R=/share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38/hg38.fa"
qsub -q short.q -b y -j y -N genotypeStep8 -hold_jid genotypeStep6 -wd $logDir -pe smp 10 -P TumourProgression -V $liftoverRecomLine


liftoverOTV="java -jar /home/nenbar/local/bin/picard.jar LiftoverVcf I=$outFolder/$projectname".OTV.b37.clean.vcf" O=$outFolder"/"$projectname".OTV.hg38.vcf" CHAIN=$annotationDir"/b37ToHg38.over.chain" REJECT=$annotationDir"/rejected_variants.vcf" R=/share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38/hg38.fa"
qsub -q short.q -b y -j y -N genotypeStep9 -hold_jid genotypeStep7 -wd $logDir -pe smp 2 -P TumourProgression -V $liftoverOTV


############## take only the samples you need and run demuxlet 
awk '{gsub(/^chr/,""); print}' $outFolder"/"$projectname".Recommended.hg38.vcf" > $outFolder"/"$projectname".Recommended.b38.vcf"
awk '{gsub(/^chr/,""); print}' $outFolder"/"$projectname".OTV.hg38.vcf" > $outFolder"/"$projectname".OTV.b38.vcf"



############## test the variables: 1. multiSNP 2. cleaning 3. imputation 
############## DEMUX RECOM ##############
...
mkdir -p $outFolder
qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectname".Recommended.b38.$projectnameSample$type.vcf" --field GT --out $outFolder"/"$stage


projectnameSample="3921_3963_4066"
type="_Blood"
#first ran the job on all samples in VCF just to see if I need subsetting
stage="recom"
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectnameSample$type/$stage"
...
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectnameSample$type/$stage"
mkdir -p $outFolder
qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectname".$stage.b38.$projectnameSample$type.vcf" --field GT --out $outFolder"/"$stage
...
#the clean data from seyhen was stored in a hg38, even
java -jar ~/local/bin/picard.jar LiftoverVcf I=$outFolder"/"$projectnameSample$type".b37.clean.vcf" O=$inFolder"/"$projectnameSample$type".hg38.$stage.vcf" CHAIN=$annotationDir"/b37ToHg38.over.chain" REJECT=$annotationDir"/rejected_variants.vcf" R=/share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38/hg38.fa
awk '{gsub(/^chr/,""); print}' $inFolder"/"$projectnameSample$type".hg38.$stage.vcf" > $inFolder"/"$projectnameSample$type".b38.$stage.vcf"

qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectnameSample$type".b38.$stage.vcf" --field GT --out $outFolder"/"$stage

#qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /paella/TumourProgressionGroupTemp/projects/data/cellranger_count/human_breast/CID4530/count_CID4530_GRCh38/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectnameSample$type".b38.$stage.vcf" --field GT --out $outFolder"/"$stage".4530test"
...
#the clean data from seyhen was stored in a hg38, even
java -jar ~/local/bin/picard.jar LiftoverVcf I=$outFolder"/setREFallele/setREFallele.vcf" O=$inFolder"/GWCCG.hg38.vcf" CHAIN=$annotationDir"/b37ToHg38.over.chain" REJECT=$annotationDir"/rejected_variants.vcf" R=/share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38/hg38.fa
awk '{gsub(/^chr/,""); print}' $inFolder"/GWCCG.hg38.vcf" > $inFolder"/"$projectnameSample$type".b38.vcf"

qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectnameSample$type".b38.vcf" --field GT --out $outFolder"/"$stage
...
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectnameSample$type/$stage"
mkdir -p $outFolder
qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 20 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder"/"$projectnameSample$type".b38.vcf" --field GT --out $outFolder"/"$stage
...
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectnameSample$type/$stage"
mkdir -p $outFolder
qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam.bam --vcf $inFolder/merge_overlapped_fullheader.vcf --field GT --out $outFolder"/"$stage
...
for i in {1..9};do
	inBam="/share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_RG.bam"
	outBam="/share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_$i.bam"
	#qsub -q short.q -b y -j y -N ss$i -hold_jid $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V "samtools view -bs 42.$i $inBam > $outBam"
	qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_$i.bam --vcf $inFolder/merge_overlapped_fullheader.vcf --field GT --out $outFolder"/"$stage"_"$i

done


############### modify read group annotation
i=1
...
inBam="/share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_$i.bam"
outBam="/share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_RG.bam"

java -jar ~/local/bin/picard.jar AddOrReplaceReadGroups I=$inBam O=$outBam RGID=1  RGLB=lib1 RGPL=illumina RGPU=unit1 RGSM="test"
...
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectnameSample$type/$stage"
mkdir -p $outFolder
qsub -q short.q -b y -j y -N $stage$projectnameSample -wd $logDir -pe smp 10 -P TumourProgression -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/$projectnameSample/outs/possorted_genome_bam_RG.bam --vcf $inFolder/merge_overlapped_fullheader.vcf --field GT --out $outFolder"/"$stage"_RG"


################check demuxlet data

projectname="demuxlet"
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectname/step2"

qsub -q short.q -b y -j y -N demuxlet -wd $logDir -pe smp 20 -P TumourProgression -V demuxlet --alpha 0 --alpha 0.5 --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/genotype/test2/merged.sorted.bam --vcf $outFolder"/"$projectname".b38.vcf" --field GT --group-list /share/ScratchGeneral/nenbar/projects/CITE/raw_files/genotype/test2/barcodes.tsv --out $outFolder"/"$projectname



projectnameTest="4513.4530"
#first ran the job on all samples in VCF just to see if I need subsetting
...
qsub -q short.q -b y -j y -N genotypeStep4 -hold_jid genotypeStep3 -wd $logDir -pe smp 20 -P TumourProgression -V
...
java -jar ~/local/bin/picard.jar LiftoverVcf I=$outFolder/$projectname".Recommended.b37.vcf" O=$outFolder"/"$projectname".Recommended.hg38.vcf" CHAIN=$annotationDir"/b37ToHg38.over.chain" REJECT=$annotationDir"/rejected_variants.vcf" R=/share/ClusterShare/biodata/contrib/nenbar/genomes/human/hg38/hg38.fa
...
vcftools --vcf 4513_4530_Blood.b37.clean.vcf --plink --out $INPUT
...
plink --bfile ${INPUT10}"_noambiguous" --a2-allele Force-Allele1-subset_chr1to22-HRC.txt --make-bed --out ${OUTPUT10}/${OUTPUT10}
...
for i in {1..22}; do unzip -P dEuGXHe7OSw5r chr_$i.zip ; done
gunzip *.dose.vcf.gz
...
qsub -q short.q -b y -j y -N demuxlet -wd $logDir -pe smp 20 -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/genotype/test/merged.sorted.bam --vcf $outFolder"/merge_overlapped_fullheader.vcf" --field GT --group-list /share/ScratchGeneral/nenbar/projects/CITE/raw_files/genotype/test/barcodes.tsv --out $outFolder"/"$projectname"_overlapped_clean"

projectname="3921_3963_4066_Blood"
outFolder="/share/ScratchGeneral/nenbar/projects/CITE/genotyping/$projectname/step2"

qsub -q short.q -b y -j y -N demuxlet -wd $logDir -pe smp 20 -V demuxlet --sam /share/ScratchGeneral/nenbar/projects/CITE/raw_files/3921_3963_4066/outs/possorted_genome_bam.bam --vcf $outFolder"/merge_overlapped.vcf" --field GT --group-list /share/ScratchGeneral/nenbar/projects/CITE/raw_files/3921_3963_4066/outs/raw_gene_bc_matrices/GRCh38/barcodes.tsv --out $outFolder"/"$projectname"_overlapped"
