...
projectname="4471_4513_tumour"
system(paste0("mkdir -p ",figDir))

#make barcodesl

seurat4513 <- Read10X("../raw_files/4471_4513/outs/4513/")
seurat4471 <- Read10X("../raw_files/4471_4513/outs/4471/")
...
citeCells4471 <- CreateSeuratObject(
    counts = seurat4471, 
    min.cells = 3, 
    min.features = 100, 
    project = projectname
...
bc4471<-colnames(citeCells4471)
bc4513<-paste0(bc4513,"-1")
bc4471<-paste0(bc4471,"-1")
bcOL<-bc4513[bc4513 %in% bc4471]

bc<-unique(c(bc4513,bc4471))
bc<-bc[!(bc %in% bcOL)]

bc4513U<-bc4513[!(bc4513 %in% bcOL)]
bc4471U<-bc4471[!(bc4471 %in% bcOL)]


totalBC<-bc
write(totalBC,file="../raw_files/4471_4513/outs/barcodes.tsv")
write(bc4513,file="../raw_files/4471_4513/outs/barcodes4513.tsv")
write(bc4471,file="../raw_files/4471_4513/outs/barcodes4471.tsv")




inFolders<-list.files(paste0("../genotyping/",projectname),full.names=T)
inFolders<-inFolders[grepl("doublet",inFolders)]
...
for(inFolder in inFolders){
  ...
  #df$bc<-gsub("-1","",df$bc)
  df$id<-gsub("\\w{22}_\\w{3}_\\w{3}_\\w{10}_","",df$id)
  df$id<-gsub(".CEL","",df$id)
  ...
  count4513<-sum(df$bc[df$id %in% "4513"] %in% bc4513)
  percent4513<-count4513/length(df$bc[df$id %in% "4513"])

  count4471<-sum(df$bc[df$id %in% "4471"] %in% bc4471)
  percent4471<-count4471/length(df$bc[df$id %in% "4471"])

  results[[basename(inFolder)]]<-c(percent4513,percent4471)
  cellCount[[basename(inFolder)]]<-c(mean(data$RD.UNIQ[-1]),mean(data$N.SNP[-1]))
  precision[[basename(inFolder)]]<-c(count4513+count4471)/dim(df)[1]

}

df<-do.call("cbind",results)
row.names(df)<-c("4513","4471")
...
for(fileName in files){
  ...
  df<-data.frame(bc=data$BARCODE,id=data$BEST)
  df$id<-gsub("5504844386755030621095_UKB_E04_YAN6909A51_","",df$id)
  df$id<-gsub("5504844386755030621095_UKB_B01_YAN6909A12_","",df$id)
  df$id<-gsub("5504844386755030621095_UKB_B08_YAN6909A19_","",df$id)
  df$id<-gsub("_Blood.*","",df$id)
  df$id<-gsub("AMB-.*","AMB",df$id)
  df$id<-gsub("SNG-","",df$id)
  ...
  cat(count4471)
  cat("\n")  
  results[[basename(fileName)]]<-c(percent4513,percent4471)
  cellCount[[basename(fileName)]]<-c(mean(data$RD.UNIQ[-1]),mean(data$N.SNP[-1]))
  precision[[basename(fileName)]]<-c(count4513+count4471)/dim(df)[1]
...
if(!file.exists(paste0(RobjectsDir,"4513_4471_miniatlas.Rdata"))){
  finalObject<-paste0(RobjectsDir,"03_seurat_CCA_aligned_processed.Rdata")
  citeCells<-readRDS(finalObject)
  #select 4513/4530
  cells<-colnames(citeCells)[grepl("4513|4471",colnames(citeCells))]
  citeCellsShort<-subset(citeCells,cells=cells)
  citeCells<-citeCellsShort
  saveRDS(citeCells,file=paste0(RobjectsDir,"4513_4471_miniatlas.Rdata"))
} else {citeCells=readRDS(paste0(RobjectsDir,"4513_4471_miniatlas.Rdata"))}


#select cell types and calculate percentage for each cell type 

########### split cells by cluster, calculate average percentage correct
#projectname="4513_4471_Blood"
inFolder=inFolders[3]
