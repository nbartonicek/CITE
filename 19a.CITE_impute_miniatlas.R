...
cellTypes<-c("B_cells_Plasma_cells", "STROMAL_with_lymphatics","B_cells", "Plasma_cells", "STROMAL", "CAFs", "Endo", "vSMCs", "Myeloid_cells", "T_cells")
#cellTypes<-c("STROMAL_with_lymphatics")
ssBCs<-list()
...
if(!file.exists(ABsListFile)){
	for(sampleName in gsub("CID","",SSsampleNames_CITE)){
		...
		write.table(df,paste0(sampleName,"_sorted_CITE_abs.txt"),quote=F,row.names=F,sep="\t")
...
imputeCITE<-function(cells,cellType,norm){
	...
	citeAll<-citeAll[rowSums(citeAll)>0,]
...
imputedCellsMiniatlasFile<-paste0(RobjectsDir,"imputed_alltypes_newAnnotation.Rdata")
result<-list()
...
#find markers
		...
		print(p)
		dev.off()
		png(paste0(outDir,"imputed_CITE_clusters_",cellType,"_",type,".png"),width=12,height=20,res=300,units='in')
		p<-DoHeatmap(cells, features = unique(adt.markers$gene),raster=F, assay = "ADT", angle = 90) + NoLegend()
		print(p)
		dev.off()		
		#plot all cite data
		...
		for(i in 1:nPlots){
			...
			print(p)
			dev.off()
			png(paste0(outDir,"feature_plot_cite_",paste0(cellType,"_",type),"_",i,".png"),width=8,height=8,res=300,units='in')
			p<-FeaturePlot(cells, features = deABs[range1:range2], min.cutoff = "q05", max.cutoff = "q95", 
			    ncol = 4, cols = c("lightgrey", "blue"), reduction = reductionsUmap[cellType], pt.size = 0.5)
			print(p)
			dev.off()
...
for(cellType in cellTypes){
	...
	for(norm in c(FALSE,TRUE)){
		...
		for(i in 1:nPlots){
			...
			print(p)
			dev.off()
			png(paste0(outDir,"feature_plot_CITE-based_",paste0(cellType,"_",type),"_",i,".png"),width=8,height=8,res=300,units='in')
			p<-FeaturePlot(cells, features = deABs[range1:range2], min.cutoff = "q05", max.cutoff = "q95", 
			    ncol = 4, cols = c("lightgrey", "blue"), reduction = "tsne_adt", pt.size = 0.5)
			print(p)
			dev.off()
