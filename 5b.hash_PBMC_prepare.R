...
pbmc_umi_sparse <- readRDS(paste0(rawDir,"pbmc_umi_mtx.rds"))
pbmc_hto <- readRDS(paste0(rawDir,"pbmc_hto_mtx.rds"))
...
raw_data=as.data.frame(as.matrix(pbmc_hashtag@raw.data))
...
writeMM(obj = pbmc_umi_sparse, file=paste0(rawDir,"matrix.mtx"))
...
class=pbmc_hashtag@meta.data$hto_classification_global
class[class=="Negative"]="SNG"
class[class=="Singlet"]="SNG"
class[class=="Doublet"]="DBL"

df<-data.frame(Barcode=colnames(pbmc_hashtag@data),Call=class)
